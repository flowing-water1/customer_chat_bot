...
book_car_rental_prompt = ChatPromptTemplate.from_messages(
    [
        ("system",
         "You are a specialized assistant for habuilder.adndling car rental bookings. "
...
def tool_looper(state: State) -> dict:
    # Retrieve the tool call stack
    tool_calls_stack = state.get("tool_calls_stack", [])
    if not tool_calls_stack:
        return {
            "messages": [
                {
                    "role": "system",
                    "type": "tool_call_response",
                    "content": "All tool calls processed.",
                    "tool_call_id": None
                }
            ]
        }

    # Get the next tool call ID to process
    tool_call_id = tool_calls_stack.pop(0)
        ...
        "messages": [
            {
                ...
                "type": "tool_call",
                "tool_call_id": tool_call_id,
                "content": f"Processing tool call with ID {tool_call_id}"
            }
        ]
    }
# Ensure to reset the tool_calls_stack if it's None
def initialize_tool_call_stack(state: State):
    if "tool_calls_stack" not in state or state["tool_calls_stack"] is None:
        state["tool_calls_stack"] = []
...
builder.add_edge("leave_skill", "tool_looper")  # 更改路由到 tool_looper
...
builder.add_edge("tool_looper", "primary_assistant")
...
_printed = set()
# Main process code to ensure tool call stack is initialized
for question in tutorial_questions:
    ...
    for event in events:
        ...
        # 确保初始化工具调用栈
        initialize_tool_call_stack(config["state"])
