    ...
    try:
        for question in tutorial_questions:
            ...
            while snapshot.next:
                ...
                chat += user_input_handler(user_input)
                yield chat
...
#我注意到，这3段代码，其实都与信息的输入和输出有关，我认为应该好好处理好这其中的关系，使得输出是一个正确的列表，不会报错。我认为应该处理“input”和user_input的outputs，因为user_input_box提交的时候，其实只是一个文本，但是outputs的chat_output，接收的其实是一个列表
#
# 1.            while snapshot.next:
#                 chat.append(("ai", "Do you approve of the above actions? Type 'y' to continue; otherwise, explain your requested change.\n\n"))
#                 yield chat
#
#                 user_input = yield "input"
...
#                 yield chat
#
# 2.            for chat in chat_generator:
#                 if chat == "input":
#                     return chat  # 继续显示输入框
#                 yield chat
#
# 3.user_input_box = gr.Textbox(label="Your Input")
#
#         user_input_box.submit(lambda user_input: user_input_handler(user_input), inputs=[user_input_box], outputs=chat_output)
