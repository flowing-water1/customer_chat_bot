...
def run_chat(model_name, api_key, api_base, tutorial_questions, user_input_handler):
    ...
    _printed = set()
    chat = []
    waiting_for_input = False  # 添加状态变量
        ...
        for question in tutorial_questions:
            ...
            while snapshot.next:
                ...
                if user_input is not None:
                    ...
                    waiting_for_input = False  # 重置状态变量
...
def main_interface():
    with gr.Blocks() as demo:
        ...
        with gr.Column(visible=False) as api_keys_section:
            with gr.Tab("OpenAI"):
                openai_api_key = gr.Textbox(label="OpenAI API Key",value = "sk-usqU5KXzBCpOP2T881D9A21838Fe47Ed8f64Ce753e192aE3", type="password")
            ...
            with gr.Row():
                langchain_api_key = gr.Textbox(label="Langchain API Key",value="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", type="password")
                tavily_api_key = gr.Textbox(label="Tavily API Key",value="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", type="password")
        ...
        with gr.Column(visible=False) as user_input_section:
        ...
        def on_start(openai_api_key, zhipu_api_key, openai_api_base, zhipu_api_base, model_name, user_input):
            ...
            for chat in chat_generator:
                if chat == "input":
                    yield {
                        user_input_section: gr.update(visible=True),
                        chat_output: gr.update()
                    }
                else:
                    yield {
                        chat_output: gr.update(value=chat),
                        user_input_section: gr.update(visible=False)
                    }


        def user_input_handler(user_input):
            return user_input
        start_button.click(on_start, inputs=[openai_api_key, zhipu_api_key, openai_api_base, zhipu_api_base, model_name, user_input_box], outputs=[chat_output, user_input_section])

        submit_button.click(
            lambda user_input: user_input,
            inputs=user_input_box,
            outputs=user_input_box
        )
